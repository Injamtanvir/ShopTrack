  ...
  static Future<dynamic> generatePriceListPdf({
    ...
    if (kIsWeb) {
      ...
      try {
        ...
        print('Error creating price list PDF: $e');
        // Return bytes as fallback
        final bytes = await pdf.save();
        return WebPdfResult(
          bytes: bytes,
          filename: 'price_list_${shopName.replaceAll(' ', '_')}.pdf',
        );
      }
    }
  }

  /// Shows sharing options based on platform
  static void showSharingOptions(BuildContext context, dynamic fileOrWebResult, String title) {
    if (kIsWeb) {
      // For web platform, just show a message since we can't use dart:html
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('PDF generated. Direct viewing is not available in this release.')),
      );
    } else {
      // For mobile platforms
      if (fileOrWebResult is File) {
        _showMobileSharingOptions(context, fileOrWebResult, title);
      } else {
        // Show a message if we can't share on this platform
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('PDF generated but sharing not available in this context')),
        );
      }
    }
  }

  // Mobile-specific method to show sharing options
  static void _showMobileSharingOptions(BuildContext context, File file, String title) {
    showModalBottomSheet(
      context: context,
      shape: const RoundedRectangleBorder(
        borderRadius: BorderRadius.vertical(top: Radius.circular(20)),
