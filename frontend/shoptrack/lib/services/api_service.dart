...
class ApiService {
  ...
  Future<Map<String, dynamic>> login({
    ...
    try {
      ...
      // Print and analyze the user data structure
      print('Login response data structure: ${data.keys}');
      if (data.containsKey('user')) {
        print('User data from server: ${data['user']}');

        // Check if designation and seller_id are in the expected location
        if (data['user'] is Map) {
          print('Designation from server: ${data['user']['designation']}');
          print('Seller ID from server: ${data['user']['seller_id']}');
        }
      }
      ...
      // Serialize the complete user data
      final userJson = jsonEncode(data['user']);
      print('Saving user data to storage: $userJson');
  ...
  // Get current user from storage with enhanced debugging
  Future<User?> getCurrentUser() async {
    try {
      final userData = await _storage.read(key: 'user');
      if (userData == null) {
      ...
      // Check if the seller_id and designation fields exist
      print('Fields in user data:');
      ...
      // Log the created user object for verification
      print('Created user object:');
      print('  Name: ${user.name}');
      print('  Email: ${user.email}');
      print('  Role: ${user.role}');
      print('  Designation: ${user.designation}');
      print('  Seller ID: ${user.sellerId}');
  ...
  // Get raw user data from storage (for debugging)
  ...
  // Verify and decode token payload directly
