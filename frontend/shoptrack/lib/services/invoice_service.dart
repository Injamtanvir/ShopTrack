...
class InvoiceService {
  ...
  Future<void> deletePendingInvoice(String invoiceId) async {
    ...
    try {
      print('Deleting invoice: ${ApiConstants.deletePendingInvoice}$invoiceId/');

      final response = await http.delete(
        Uri.parse('${ApiConstants.deletePendingInvoice}$invoiceId/'),
        headers: {
          'Authorization': 'Bearer $token',
        },
      );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          print('Error parsing pending invoices: $parseError');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          print('Error parsing invoice history data: $parseError');
          throw Exception('Failed to parse invoice data: $parseError');
        }
      } else {
        if (response.body.contains('<!DOCTYPE') || response.body.contains('<html>')) {
